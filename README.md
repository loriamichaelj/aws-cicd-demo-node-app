...
(`rollback.yml`) is confirmed working on `dev`; `stage`/`prod` rollback untested for this
app specifically (identical code path already proven on `python-app`'s `stage`).
